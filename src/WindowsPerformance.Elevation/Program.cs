...
if (args.Length == 0)
{
    await Console.Error.WriteLineAsync("Verwendung: ElevationHelper.exe --server | <Base64Script> [--timeout <ms>]");
    return 1;
}
...
var result = await ExecuteScriptAsync(scriptBase64, timeoutMs);
...
static async Task RunServerAsync(JsonSerializerOptions jsonOptions)
{
    ...
    while (!cts.Token.IsCancellationRequested)
    {
        ...
        var response = await ExecuteScriptAsync(request?.Script ?? string.Empty, request?.TimeoutMs ?? 30000);
        await writer.WriteLineAsync(JsonSerializer.Serialize(response, jsonOptions));
    }
}

static async Task<ElevationResponse> ExecuteScriptAsync(string scriptBase64, int timeoutMs)
{
